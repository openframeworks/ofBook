...
for chapter in chapters:
	...
	if os.path.exists(destChapterPath):
		print("destChapterPath=", destChapterPath)
		soup = Soup(open(destChapterPath, 'rb').read(),"html5lib")
		...
		if len(divFigures) != 0:
			...
			for fig in divFigures:
				...
				imgHyperlink = Soup("<a href=""></a>").a
				imgHyperlink.href=fig.img["src"]
				fig.img.wrap(imgHyperlink)


				fig['class'] = "inner"
				divWhole = Soup("<div></div>").div
...
for idx, chapter in enumerate(chapterDicts): 
	soup = Soup("", "html5lib")
	ff = open(chapter['destChapterPath'], "rb")
	soupFromFile = Soup(ff.read(), "html5lib")
	ff.close()
	# Create previous/next links in the footer
	nextChapterDiv = soupFromFile.find("div", id='next_chapter')
	prevChapterDiv = soupFromFile.find("div", id='prev_chapter')
#	print(nextChapterDiv)
#	print(prevChapterDiv)
	if (idx != 0):
		prevLink = Soup("<a></a>").a
	...
	for group in chapterGroups:
		...
		for chap in group['chapters']:
			...
			if (c != None):
				chapUl = Soup("<ul></ul>").ul
				groupLi.append(chapUl)

				chapLi = Soup("<li></li>").li
				...
				if (len(c['sections'])):
					...
					ulInner = Soup("<ul></ul>").ul
					divTag = Soup("<div></div>").div
	...
	#htmlFromFile = soupFromFile.encode(encoding='utf-8')
	
	with open(chapter['destChapterPath'], "wb") as file:
		file.write(soupFromFile.encode(encoding='utf-8'))
		file.flush()
		file.close()
	#print html
