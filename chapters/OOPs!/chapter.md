...
Going back to the initial version of our `Ball` class we'll build some 'daughter' classes based on its main characteristics (motion behaviors and shape) but we'll distinguish each inherited sub-class by using a different color on its drawing method.
